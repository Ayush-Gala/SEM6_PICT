...
if (empty($fname_err) && empty($lname_err) && empty($email_err) && empty($course_err) && empty($batch_err) &&
empty($city_err) && empty($state_err)) {
// Prepare a update statement
$sql = "UPDATE students SET firstname = ?, lastname = ?, email = ?, course = ?, batch = ?, city = ?, state = ? WHERE
id = ? ";
...
<div class="col-lg-6">
<!-- ��� form start �� -->
<form action="<?= htmlspecialchars(basename($_SERVER["REQUEST_URI"])); ?>" method="post" class="bg-light p-4
